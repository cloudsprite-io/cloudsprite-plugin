...
2. Use `search_sdk` for the requested operation, then `inspect_sdk` on the
   exact returned method. Check its REST binding, path/query parameters,
   request body, response shape, permissions, and execution restrictions.
   Call `use_sdk` with that method and the inspected arguments. REST
   operation IDs and guessed Python names are not callable method names.
...
- Read the actual notebook and its linked datasets, or resolve the user's
  ...
  relevant parameters, units, conditions, and trace inventory. Use existing
  read tools such as `get_notebook`, `get_dataset`, and `get_trace_summary`;
  discover additional read methods through the SDK if needed.
- Notebook membership is not a measurement result. Use trace summaries or
  ...
  notebook's frozen results. Never fetch raw waveform arrays into context.
...
- Cite evidence next to the claim with `[[slug]]`, replacing `slug` with the
  exact slug returned for that source. Cite the underlying dataset as well
  as the notebook when the claim uses dataset evidence. Never turn a title
  or UUID into a made-up slug. If a slug cannot be resolved, identify the
  source by name/ID in plain text and say its linked citation is unavailable.
...
- Create a saved **draft** using the inspected create contract and resolved
  project. Use the proposed title and evidence-backed Markdown. On a title
  conflict, resolve which existing report or new title the user intends;
  never overwrite a similarly named report automatically.
...
- Saving the body extracts real references from valid citations. Read back
  the saved body and available references to verify resolution. Report broken
  or unresolved citations; do not claim working backlinks merely because a
  token appears in Markdown. Auto references are changed by editing citations;
...
does not rewrite history or mean "restore from trash." Preserve historical
references to missing sources and report them as broken rather than deleting
them. Ordinary autosave is not a version snapshot.
...
"yes." Permanent deletion remains denied. Version restore has no typed-name
guard, but still requires user intent, the current precondition, and RBAC.
